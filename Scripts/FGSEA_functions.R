...
load.MSigDB <- function(species) {
  ...
  print(
    dplyr::distinct(msig_df, gs_collection, gs_subcollection) %>%
...
run.FGSEA <- function(rnk, genesets, nproc = 2, minGenes = 3, maxGenes = 5000, minOverlap = 1, reformat = T, filename = F, minP = 1e-20, ...){
	...
	if(reformat){
		...
		ord_gsea_res$overlap <- lengths(regmatches(ord_gsea_res$leadingEdge, gregexpr(",", ord_gsea_res$leadingEdge))) + 1
		ord_gsea_res <- ord_gsea_res[ord_gsea_res$overlap >= minOverlap,]
		...
		ord_gsea_res$overlap <- lengths(ord_gsea_res$leadingEdge)
		ord_gsea_res <- ord_gsea_res[ord_gsea_res$overlap >= minOverlap,]
		if(is.character(filename)){
...
#' @export filter.FGSEA
filter.FGSEA <- function(res, minOverlap){
	return(res[res$overlap >= minOverlap,])
}
